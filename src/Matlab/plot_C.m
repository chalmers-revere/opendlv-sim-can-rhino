...
figure(2);   
subplot(4,1,1);
plot(t,P_cente(:,1),'-.',t,P_sens(:,1)),grid;
ylabel('X(m)');
title('POSITION:SENSED VS COMMAND');
 

subplot(4,1,2);
plot(t,P_cente(:,2),'-.',t,P_sens(:,2)),grid;
ylabel('Y(m)');
 xlabel('time(s)');
 
 subplot(4,1,3);
plot(  t, y1_nom(:,1), '--',  t,y1_actual(:,1) ),grid;
legend('reference','nominal', 'actual');
ylabel('v(m/s)');
 xlabel('time(s)');
 
 subplot(4,1,4);
plot( t, y1_nom(:,4)/pi*180, '--',  t,y1_actual(:,4)/pi*180 ),grid;
ylabel('\psi(degree)');
 xlabel('time(s)');
 
 figure(20); 
  subplot(2,1,1);
plot(  t, y1_nom(:,2), '--',  t,y1_actual(:,2) ),grid;
legend( 'nominal', 'actual');
ylabel('v_y(m/s)');
 xlabel('time(s)');
 
 subplot(2,1,2);
plot(  t, y1_nom(:,3), '--',  t,y1_actual(:,3) ),grid;
